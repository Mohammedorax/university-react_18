...
export default function DiscountsPage() {
  ...
  const [confirmDialog, setConfirmDialog] = useState<{
    open: boolean; discountId?: string; bulk?: Discount[]; description?: string
  }>({ open: false })
  const limit = 9
  ...
  const handleDelete = useCallback((id: string) => {
    setConfirmDialog({ open: true, discountId: id, description: 'هل أنت متأكد من حذف هذا الخصم؟' })
  }, [])

  /**
   * حذف مجموعة من الخصومات
   */
  const handleBulkDelete = useCallback((selected: Discount[]) => {
    setConfirmDialog({ open: true, bulk: selected, description: `هل أنت متأكد من حذف ${selected.length} خصومات؟` })
  }, [])

  const handleConfirmDelete = useCallback(async () => {
    try {
      if (confirmDialog.bulk) {
        await Promise.all(confirmDialog.bulk.map(d => deleteDiscountMutation.mutateAsync(d.id)))
        toast.success('تم حذف الخصومات المحددة بنجاح')
        setSelectedDiscounts([])
      } else if (confirmDialog.discountId) {
        await deleteDiscountMutation.mutateAsync(confirmDialog.discountId)
        toast.success('تم حذف الخصم بنجاح')
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'حدث خطأ أثناء عملية الحذف';
      toast.error(errorMessage)
    } finally {
      setConfirmDialog({ open: false })
    }
  }, [confirmDialog, deleteDiscountMutation])
  ...
  const columns: DataTableColumn<Discount>[] = [
    ...
    },
    {
      ...
      sortable: true,
      render: (val: string) => (
        <Badge variant={val === 'percentage' ? 'default' : 'secondary'}>
    ...
    },
    {
      ...
      sortable: true,
      render: (val: boolean) => (
        <Badge className={val ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 dark:bg-emerald-500/20' : 'bg-destructive/10 text-destructive dark:bg-destructive/20'}>
          {val ? 'نشط' : 'غير نشط'}
        </Badge>
      )
    }
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-background pb-10">
        <div className="relative overflow-hidden bg-gradient-to-br from-primary via-primary/90 to-primary/70 dark:from-primary/90 dark:via-primary/80 dark:to-primary/60 pb-32 pt-12">
  ...
  return (
    <>
      <div className="min-h-screen bg-background pb-10" dir="rtl" role="main" aria-labelledby="discounts-title">
        ...
        {/* Hero Section */}
        <div className="relative overflow-hidden bg-gradient-to-br from-primary via-primary/90 to-primary/70 dark:from-primary/90 dark:via-primary/80 dark:to-primary/60 text-primary-foreground pb-32 pt-12 shadow-2xl">
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-10">
              <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-right">
                <div className="bg-primary-foreground/10 dark:bg-primary-foreground/15 backdrop-blur-md p-5 rounded-3xl border border-primary-foreground/20 shadow-2xl" aria-hidden="true">
                  <Percent size={56} className="text-primary-foreground" />
                </div>
                <div>
                  <h1 id="discounts-title" className="text-3xl md:text-5xl font-black mb-3 tracking-tight text-primary-foreground">إدارة الخصومات</h1>
              ...
              <div className="flex items-center gap-3">
                <Button
                  ...
                  variant="secondary"
                  className={cn(
                    "h-12 w-12 rounded-2xl shadow-xl transition-all active:scale-95 bg-primary-foreground/20 hover:bg-primary-foreground/30 text-primary-foreground border-primary-foreground/20",
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {paginatedDiscounts.map((discount) => (
                      ...
                      >
                        <div className={cn(
                          "h-2 w-full",
                          discount.type === 'percentage' ? 'bg-gradient-to-r from-primary to-primary/70' : 'bg-gradient-to-r from-primary/80 to-primary/50'
                        )} aria-hidden="true" />
                          ...
                          <div className="flex justify-between items-start mb-4">
                            <Badge className={cn(
                              ...
                              discount.type === 'percentage'
                                ...
                                : 'bg-secondary/50 text-secondary-foreground dark:bg-secondary/20 hover:bg-secondary/60'
                            )}>
                            ...
                            <div className="flex gap-1 transition-all duration-300">
                              <Button
                                ...
                                className={cn(
                                  ...
                                  discount.active ? 'text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10' : 'text-muted-foreground hover:bg-muted'
                                )}
                        ...
                        <CardContent className="pb-8 pt-2 px-6">
                          <div className="flex items-center gap-4 mb-6 p-4 bg-muted/50 rounded-2xl border border-border group-hover:bg-card group-hover:border-primary/20 transition-all duration-300">
                            <div className={cn(
                              "p-3 rounded-xl",
                              discount.type === 'percentage'
                                ? 'bg-gradient-to-br from-primary to-primary/70 text-primary-foreground shadow-lg shadow-primary/20 dark:shadow-primary/10'
                                : 'bg-gradient-to-br from-primary/80 to-primary/50 text-primary-foreground shadow-lg shadow-primary/15 dark:shadow-primary/10'
                            )} aria-hidden="true">
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex -space-x-2 rtl:space-x-reverse" aria-label="أمثلة لطلاب مستفيدين">
                              {[1, 2, 3].map(i => (
                                <div key={i} className="h-8 w-8 rounded-full border-2 border-background bg-gradient-to-br from-primary to-primary/70 dark:from-primary/90 dark:to-primary/60 flex items-center justify-center text-[10px] font-bold text-primary-foreground shadow-sm" aria-hidden="true">
                                  {i}
                                </div>
                              ))}
                              <div className="h-8 w-8 rounded-full border-2 border-background bg-muted dark:bg-muted/50 flex items-center justify-center text-[10px] font-black text-muted-foreground" aria-label="وأكثر من اثني عشر طالباً آخر">
                                +12
                              </div>
                            </div>
                            ...
                            {!discount.active ? (
                              ...
                              <div className="flex items-center gap-1.5 text-emerald-600 dark:text-emerald-400 text-xs font-bold bg-emerald-500/10 dark:bg-emerald-500/20 px-3 py-1.5 rounded-full" role="status">
