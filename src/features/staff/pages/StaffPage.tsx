...
export default function StaffPage() {
  ...
  const [confirmDialog, setConfirmDialog] = useState<{
    open: boolean; staffId?: string; bulk?: Staff[]; description?: string
  }>({ open: false })
  const limit = 10
  ...
  const handleDelete = useCallback((id: string) => {
    setConfirmDialog({ open: true, staffId: id, description: 'هل أنت متأكد من حذف هذا الموظف؟' })
  }, [])

  /**
   * حذف مجموعة من الموظفين
   */
  const handleBulkDelete = useCallback((selectedStaff: Staff[]) => {
    setConfirmDialog({ open: true, bulk: selectedStaff, description: `هل أنت متأكد من حذف ${selectedStaff.length} موظفين؟` })
  }, [])

  const handleConfirmDelete = useCallback(async () => {
    try {
      if (confirmDialog.bulk) {
        await Promise.all(confirmDialog.bulk.map(s => deleteStaffMutation.mutateAsync(s.id)))
        toast.success('تم حذف الموظفين المحددين بنجاح')
        setSelectedStaff([])
      } else if (confirmDialog.staffId) {
        await deleteStaffMutation.mutateAsync(confirmDialog.staffId)
  ...
  }, [confirmDialog, deleteStaffMutation])
