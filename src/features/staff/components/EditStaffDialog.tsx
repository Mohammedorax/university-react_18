 ...
 * @description نافذة منبثقة لتعديل بيانات موظف موجود.
 * تستخدم React Hook Form مع Zod للتحقق من صحة البيانات وتوفر واجهة مستخدم متناسقة مع نموذج الإضافة.
 */
export function EditStaffDialog({ staff, trigger }: EditStaffDialogProps) {
  ...
  // تحديث قيم النموذج عند تغيير الموظف أو فتح النافذة
  useEffect(() => {
    if (isOpen) {
      form.reset({
        name: staff.name,
  ...
  const onSubmit = async (values: StaffFormValues) => {
    try {
      ...
      const message = error instanceof Error ? error.message : "فشل في تحديث بيانات الموظف"
      toast.error(message);
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        {trigger || (
          <Button 
            ...
            aria-label={`تعديل بيانات الموظف ${staff.name}`}
          >
            تعديل
          </Button>
        )}
      ...
      <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden border-none shadow-2xl">
        <DialogHeader className="p-6 bg-primary text-primary-foreground">
          <DialogTitle className="text-2xl font-black">تعديل بيانات الموظف</DialogTitle>
          <DialogDescription className="text-primary-foreground/80 font-medium">
            قم بتعديل بيانات الموظف {staff.name} هنا. تأكد من صحة التعديلات قبل الحفظ.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="p-6 space-y-6 bg-background">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-job_title" className="text-sm font-bold text-foreground/70 mr-1">المسمى الوظيفي</Label>
              <Input 
                id="edit-job_title" 
            ...
            <div className="space-y-2 md:col-span-2">
              <Label htmlFor="edit-phone" className="text-sm font-bold text-foreground/70 mr-1">رقم الهاتف</Label>
              <Input 
                id="edit-phone" 
                {...form.register("phone")}
