...
export default function StudentsPage() {
  ...
  const handleDelete = useCallback(async (id: string) => {
    if (window.confirm('هل أنت متأكد من حذف هذا الطالب؟ سيتم حذف جميع البيانات المرتبطة به.')) {
      try {
  ...
  /**
   * معالج حذف مجموعة من الطلاب مع التأكيد
   * 
   * @async
   * @function handleBulkDelete
   * @param {Student[]} selectedStudents - قائمة الطلاب المحددين للحذف
   * @returns {Promise<void>}
   */
  const handleBulkDelete = useCallback(async (selectedStudents: Student[]) => {
    if (window.confirm(`هل أنت متأكد من حذف ${selectedStudents.length} طلاب؟ لا يمكن التراجع عن هذا الإجراء.`)) {
      try {
        await Promise.all(selectedStudents.map(s => deleteStudentMutation.mutateAsync(s.id)))
        toast.success('تم حذف الطلاب المحددين بنجاح')
      } catch (err) {
        toast.error('حدث خطأ أثناء حذف بعض الطلاب')
      }
    }
  }, [deleteStudentMutation])
  ...
  return (
    <div className="min-h-screen bg-background pb-10" dir="rtl" lang="ar" role="main">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 -mt-16 relative z-20">
        {/* Filters Card */}
        <Card className="border-none shadow-xl bg-card/50 backdrop-blur-sm rounded-3xl overflow-hidden mb-8" role="search" aria-label="فلاتر البحث والتصفية">
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="relative group">
                ...
                <Input
                  ...
                  className="pr-12 h-12 bg-background border-input rounded-xl font-bold focus-visible:ring-2 focus-visible:ring-primary transition-all"
              ...
              <div className="relative">
                <Select value={selectedDepartment} onValueChange={setSelectedDepartment}>
                  <SelectTrigger className="w-full pr-12 h-12 bg-background border-input rounded-xl font-bold focus:ring-2 focus:ring-primary outline-none transition-all" aria-label="تصفية حسب القسم">
                    <Filter className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground" size={18} aria-hidden="true" />
              ...
              <div className="relative">
                <Select value={selectedYear} onValueChange={setSelectedYear}>
                  <SelectTrigger className="w-full pr-12 h-12 bg-background border-input rounded-xl font-bold focus:ring-2 focus:ring-primary outline-none transition-all" aria-label="تصفية حسب السنة الدراسية">
                    <BookOpen className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground" size={18} aria-hidden="true" />
